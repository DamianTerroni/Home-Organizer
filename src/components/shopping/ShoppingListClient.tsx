...
export default function ShoppingListClient({
  ...
  async function startNewList() {
    ...
    if (!confirm("¿Vaciar toda la lista (incluyendo lo pendiente) para empezar una nueva?")) return;
    await supabase.from("shopping_items").delete().eq("household_id", householdId);
  }

  async function completePurchase() {
    if (!supabase || done.length === 0) return;
    if (!confirm(`¿Marcar la compra como realizada? Se van a quitar los ${done.length} artículos ya comprados; lo que quedó pendiente se mantiene en la lista.`)) return;
